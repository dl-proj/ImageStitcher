...
from src.edge.detector import detect_edge_frame
    ...
    def _take_video(self):
        ...
        try:
            if type(self.port_num) == int:
                ...
                ret = False
            if ret:
                edge_frame = detect_edge_frame(frame=frame.copy(), thresh_value=self.thresh_value)
                edge_frame = cv2.resize(edge_frame, (int(self.width), int(self.height)))
                if self._overlay_frame is not None:
                    edge_frame = self._overlay_frame
                time.sleep(.01)
        except (cv2.error, AttributeError, ConnectionError):
            frame = None
            edge_frame = None
        self._update_video(frame=edge_frame, origin_frame=frame)

    def _update_video(self, origin_frame, frame=None, *args):
        """
